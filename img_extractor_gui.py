...
Based on img_extractor.py ObjectExtractor
"""
...
# Import ObjectExtractor from img_extractor.py
try:
    from img_extractor import ObjectExtractor
except ImportError:
...
class GUIController:
    ...
    def get_new_file_name(self, directory: str) -> str:
        """
        the function returns unique PNG-filename in the directory 

        name format:
            {timestamp}_{uuid}.png
        Example:
            1731258123456_6f1e7ad38a1b4a779bb8b27f4bde3a22.png
        """
        timestamp = int(time.time() * 1000)  # миллисекунды
        unique_id = uuid.uuid4().hex  # 32-символьный UUID без дефисов
    ...
    def extract_selected_groups(self, dilation: int = 2) -> bool:
        ...
        try:
            ...
            for group in selected:
                ...
                # Save PNG
                output_file = self.get_new_file_name(self.extractor.output_dir)
...
class MainWindow(QMainWindow):
    ...
    def create_control_panel(self) -> QWidget:
        ...
        self.padding_slider.setMaximum(20)
