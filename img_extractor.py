...
class ObjectExtractor:
    ...
    def __init__(self, input_file, strategy=1, padding=10, debug=False):
        ...
        # --- GEOMETRY CACHES ---
        # Cache for base (non-expanded) Shapely polygons
        self._cached_shapely_polygons: Dict[int, Optional[Polygon]] = {}
        # Cache for expanded/dilated objects (depends on padding and strategy)
        self._cached_expanded_geometries: Dict[int, Optional[base.BaseGeometry | np.ndarray]] = {}
        # Values for which _cached_expanded_geometries cache was created
    ...
    def find_components(self):
        ...
        self.components = []
        self._full_masks = {} # Clear mask cache

        # --- CHANGE: Complete clearing of geometry caches (and counters) ---
        ...
        for i in range(1, num_labels):
            ...
            self.components.append({
                'id': i, 'bbox': (x, y, w, h), 'area': area,
                'mask': mask, 'hull': hull, 'centroid': centroids[i]
            })
        if self.debug:
    ...
    def bbox_intersects_hull(self, bbox_a, hull_b_idx: int) -> bool:
        ...
        if not hull_polygon: return False
        try:
    ...
    def _merge_condition_s2(self, idx_a, idx_b, padding=0):
        ...
        # --- NEW OPTIMIZATION: "Coarse" check (Broad-phase) ---
        # First check BBox intersection (S1). If there is none, then HULL
        # definitely don't intersect, and there's no point in expensive check.
        if not self._merge_condition_s1(idx_a, idx_b, padding):
            return False
        # --- END OPTIMIZATION ---

        # "Fine" check (only if BBox intersected)
    ...
    # ----------------------------------------------------------------------
    # CACHE MANAGEMENT METHODS (with logging)
    # ----------------------------------------------------------------------

    def _clear_geometry_cache(self, clear_all: bool = False):
        """Clears geometry caches and resets counters."""
        # Reset counters
        self._cache_hits = 0
        self._cache_misses = 0
    ...
    def _get_shapely_polygon(self, comp_idx: int) -> Optional[Polygon]:
        """
        Returns (or creates and caches) base (non-expanded)
        Shapely polygon for component's convex hull.
        ...
        try:
            ...
            self._cached_shapely_polygons[comp_idx] = polygon
            return polygon
        except Exception:
            self._cached_shapely_polygons[comp_idx] = None
            return None
    ...
    def _get_expanded_hull_s3(self, comp_idx: int) -> Optional[Polygon]:
        """
        Returns (or creates and caches) EXPANDED Shapely polygon (S3).
        (Implements "lazy" caching)
        """
        # 1. Check cache (HIT)
        ...
        # Get base polygon (from its cache)
        ...
        try:
            ...
            self._cached_expanded_geometries[comp_idx] = expanded_polygon
    ...
    def _get_dilated_mask_s4(self, comp_idx: int) -> Optional[np.ndarray]:
        """
        Returns (or creates and caches) DILATED mask (S4).
        (Implements "lazy" caching)
        """
        # 1. Check cache (HIT)
        if comp_idx in self._cached_expanded_geometries:
        ...
        if self.padding <= 0:
            self._cached_expanded_geometries[comp_idx] = mask
            return mask
        try:
            kernel_size = 2 * self.padding + 1
            kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size))
            dilated_mask = cv2.dilate(mask, kernel, iterations=1)
            self._cached_expanded_geometries[comp_idx] = dilated_mask
            return dilated_mask
        except Exception as e:
            if self.debug:
    ...
    # ----------------------------------------------------------------------
    # HELPER METHOD: Getting full component mask
    # ----------------------------------------------------------------------
    def get_component_mask_full(self, comp_idx: int) -> np.ndarray:
        """
        Returns full binary mask of component in original image size.
        (Uses self._full_masks cache)
        """
    ...
    # ----------------------------------------------------------------------
    # Merge condition for Strategy 4 (Dilated Mask Intersection)
    # ----------------------------------------------------------------------
    def _merge_condition_s4(self, idx_a: int, idx_b: int, padding: int) -> bool:
        ...
        (Uses cache and "Broad-Phase" filtering)
        """
    ...
    # ----------------------------------------------------------------------
    # CORE GROUPING METHOD (unchanged)
    # ----------------------------------------------------------------------
    ...
    def group_components_by_strategy(self, strategy_name: str, merge_condition: Callable, padding: int) -> List[Set[int]]:
        ...
        (Uses "lazy" caching and timers)
        """
        print(f"Applying Strategy {self.strategy}: {strategy_name} (padding={self.padding})...")
        start_time = time.perf_counter()

        # --- CACHE MANAGEMENT BLOCK (Invalidation) ---
        # Check if expanded objects cache needs invalidation
        if self.padding != self._cache_padding_value or self.strategy != self._cache_strategy_value:
            ...
            # Clear only expanded objects cache AND COUNTERS
        ...
        # --- "AGGRESSIVE" CACHING REMOVED FROM HERE ---
                ...
                for j in range(i + 1, len(groups)):
                    ...
                    for idx_a in group_a:
                        for idx_b in group_b:
                            # ! merge_condition call now QUICKLY
                            # filters out 99% of pairs (for S2, S3, S4)
